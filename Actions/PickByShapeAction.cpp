...
void PickByShapeAction::Execute()
{
	if (pManager->GetFigCount() != 0)
	{
		...
		Restart:
		pOut->PrintMessage("Pick all " + pManager->ShapeString(pManager->GetPickingFig()) + " Shapes.");
		...
		while (cont)
		{
			...
			if (PickedFig != NULL)
			{
				if (!PickedFig->GetHidden())
					if (pManager->GetPickingFig()->GetShape() == PickedFig->GetShape()) pManager->IncrementCorrectPicks();
			...
			if (pManager->GetCorrectPicks() == pManager->GetPickingShapeCount())
			{
